# WebRTC Video Call Application

Веб-приложение для видеозвонков на основе WebRTC с использованием Supabase для сигнализации.

## Установка

Требуется: Node.js & npm - [установка с nvm](https://github.com/nvm-sh/nvm#installing-and-updating)

```sh
# Клонирование репозитория
git clone <YOUR_GIT_URL>

# Переход в директорию проекта
cd <YOUR_PROJECT_NAME>

# Установка зависимостей
npm i

# Запуск dev сервера
npm run dev
```

## Технологии

- Vite
- TypeScript
- React
- shadcn-ui
- Tailwind CSS
- WebRTC
- Supabase (Realtime для сигнализации)

---

## 🔴 ВАЖНО: Конфигурация TURN серверов

### Проблема с бесплатными TURN серверами

**По умолчанию приложение использует бесплатные публичные TURN серверы, которые НЕ ПОДХОДЯТ для продакшн использования!**

Бесплатные TURN серверы имеют серьезные ограничения:
- 🔴 **Ненадежность**: часто перегружены или недоступны
- 🔴 **Региональные блокировки**: могут быть заблокированы в вашей стране/регионе
- 🔴 **Низкая скорость**: ограниченная пропускная способность
- 🔴 **Нет SLA**: никаких гарантий работоспособности
- 🔴 **Лимиты трафика**: жесткие ограничения на объем данных
- 🔴 **Не работают с VPN**: часто блокируются VPN-провайдерами

### Когда нужны TURN серверы?

TURN серверы критически важны в следующих сценариях:

1. **VPN** - один или оба пользователя используют VPN
2. **Корпоративная сеть** - строгие firewall и NAT правила
3. **CGNAT** - провайдер использует Carrier-grade NAT (мобильный интернет, некоторые ISP)
4. **Симметричный NAT** - самый строгий тип NAT, блокирует P2P соединения
5. **Разные континенты** - пользователи находятся в разных странах с различной сетевой инфраструктурой

В этих случаях прямое P2P соединение **невозможно**, и весь трафик должен идти через TURN relay сервер.

---

## ✅ Решения для продакшн

### ⚙️ Конфигурация через переменные окружения (.env)

**ВАЖНО:** Все TURN credentials теперь настраиваются через `.env` файл для улучшения безопасности!

**Настройка:**

1. Скопируйте `.env.example` в `.env`:
   ```bash
   cp .env.example .env
   ```

2. Откройте файл `.env` и обновите TURN server credentials:

```env
# TURN Server Configuration
VITE_TURN_SERVER_URL="turn:your-turn-server.com:443"
VITE_TURN_SERVER_USERNAME="your-username"
VITE_TURN_SERVER_CREDENTIAL="your-password"

# Optional: Secondary TURN server (for redundancy)
VITE_TURN_SERVER_URL_2="turn:your-turn-server.com:443?transport=tcp"
VITE_TURN_SERVER_USERNAME_2="your-username"
VITE_TURN_SERVER_CREDENTIAL_2="your-password"
```

3. Перезапустите dev сервер: `npm run dev`

**Преимущества:**
- ✅ Credentials не хранятся в коде
- ✅ Легко обновлять без изменения кода
- ✅ Разные настройки для dev/staging/production
- ✅ Безопасность: `.env` уже добавлен в `.gitignore`

**Примечание для деплоя:**
- Для production деплоя настройте эти переменные в настройках вашего хостинга (Vercel, Netlify, и т.д.)
- **НЕ коммитьте** `.env` файл в git! Файл уже добавлен в `.gitignore`
- Используйте `.env.example` как шаблон для других разработчиков

---

### Вариант 1: Платные облачные TURN сервисы (рекомендуется)

#### 1.1. Metered.ca (рекомендуется для начинающих)

**Преимущества:**
- Бесплатный тариф: 50GB/месяц
- Глобальная сеть серверов
- Простая интеграция
- WebRTC-специализированный сервис

**Настройка:**
1. Регистрация: [https://www.metered.ca/](https://www.metered.ca/)
2. Получите credentials из дашборда
3. Добавьте credentials в `.env` файл:

```env
VITE_TURN_SERVER_URL="turn:a.relay.metered.ca:443"
VITE_TURN_SERVER_USERNAME="YOUR_METERED_USERNAME"
VITE_TURN_SERVER_CREDENTIAL="YOUR_METERED_CREDENTIAL"

# Дополнительный TURN для надежности
VITE_TURN_SERVER_URL_2="turn:a.relay.metered.ca:443?transport=tcp"
VITE_TURN_SERVER_USERNAME_2="YOUR_METERED_USERNAME"
VITE_TURN_SERVER_CREDENTIAL_2="YOUR_METERED_CREDENTIAL"
```

4. Перезапустите приложение

**Примечание:** Можно использовать несколько URL в VITE_TURN_SERVER_URL, разделяя их запятыми.

**Цены:** от $0 (50GB) до $99/месяц (1TB)

#### 1.2. Xirsys

**Преимущества:**
- Специализация на WebRTC
- Глобальная сеть
- Хорошая документация

**Настройка:**
1. Регистрация: [https://xirsys.com/](https://xirsys.com/)
2. Создайте канал в дашборде
3. Получите credentials
4. Добавьте в `.env` файл:

```env
VITE_TURN_SERVER_URL="turn:your-channel.xirsys.com:443"
VITE_TURN_SERVER_USERNAME="YOUR_XIRSYS_USERNAME"
VITE_TURN_SERVER_CREDENTIAL="YOUR_XIRSYS_CREDENTIAL"
```

**Цены:** от $10/месяц

#### 1.3. Twilio Network Traversal Service

**Преимущества:**
- Enterprise-уровень надежности
- Интеграция с другими Twilio сервисами
- Отличная поддержка

**Настройка:**

1. Получите TURN credentials через Twilio API
2. Добавьте в `.env` файл:

```env
# Twilio предоставляет эфемерные токены, их нужно динамически получать
# через API и затем использовать. Пример для статических credentials:
VITE_TURN_SERVER_URL="turn:global.turn.twilio.com:3478"
VITE_TURN_SERVER_USERNAME="YOUR_TWILIO_USERNAME"
VITE_TURN_SERVER_CREDENTIAL="YOUR_TWILIO_CREDENTIAL"
```

**Примечание:** Для Twilio рекомендуется получать эфемерные токены через их API перед каждым звонком для максимальной безопасности.

**Цены:** pay-as-you-go, ~$0.001 за минуту

#### 1.4. VideoSDK

**Преимущества:**
- Комплексное решение для видео
- TURN серверы включены
- Простая интеграция

**Настройка:**
1. Регистрация: [https://www.videosdk.live/](https://www.videosdk.live/)
2. Получите TURN credentials
3. Добавьте в `.env` файл:

```env
VITE_TURN_SERVER_URL="turn:turn.videosdk.live:3478"
VITE_TURN_SERVER_USERNAME="YOUR_VIDEOSDK_USERNAME"
VITE_TURN_SERVER_CREDENTIAL="YOUR_VIDEOSDK_CREDENTIAL"
```

**Цены:** от бесплатного тарифа

---

### Вариант 2: Собственный coturn сервер (максимальный контроль)

**Преимущества:**
- Полный контроль
- Нет лимитов трафика
- Конфиденциальность данных
- Однократные расходы на VPS

**Требования:**
- VPS с минимум 2GB RAM, 2 CPU cores
- Ubuntu 20.04+ или Debian 11+
- Публичный IP адрес
- Открытые порты: 3478 (UDP/TCP), 5349 (UDP/TCP), 49152-65535 (UDP)

#### Установка coturn на Ubuntu

```bash
# 1. Установка coturn
sudo apt update
sudo apt install coturn -y

# 2. Включение coturn
sudo nano /etc/default/coturn
# Раскомментируйте: TURNSERVER_ENABLED=1

# 3. Конфигурация
sudo nano /etc/turnserver.conf
```

**Минимальная конфигурация `/etc/turnserver.conf`:**

```conf
# Прослушивание
listening-ip=0.0.0.0
listening-port=3478
tls-listening-port=5349

# Публичный IP вашего сервера
external-ip=YOUR_SERVER_PUBLIC_IP

# Диапазон relay портов
min-port=49152
max-port=65535

# Realm (домен вашего приложения)
realm=yourdomain.com

# Логирование
verbose
log-file=/var/log/turnserver.log

# Аутентификация
lt-cred-mech
user=username:password

# Для защиты используйте fingerprint
fingerprint

# База данных пользователей (опционально)
# userdb=/var/lib/turn/turndb

# Ограничения (против DDoS)
max-bps=1000000
total-quota=100
```

**Настройка SSL/TLS (для порта 5349):**

```bash
# Используйте Let's Encrypt
sudo apt install certbot -y
sudo certbot certonly --standalone -d turn.yourdomain.com

# Укажите в /etc/turnserver.conf:
# cert=/etc/letsencrypt/live/turn.yourdomain.com/cert.pem
# pkey=/etc/letsencrypt/live/turn.yourdomain.com/privkey.pem
```

**Запуск:**

```bash
sudo systemctl restart coturn
sudo systemctl enable coturn

# Проверка статуса
sudo systemctl status coturn

# Проверка логов
sudo tail -f /var/log/turnserver.log
```

**Тестирование coturn:**

Используйте онлайн тест: [https://webrtc.github.io/samples/src/content/peerconnection/trickle-ice/](https://webrtc.github.io/samples/src/content/peerconnection/trickle-ice/)

Или с помощью утилиты:
```bash
npm install -g turn-test
turn-test YOUR_SERVER_IP 3478 username password
```

**Конфигурация в приложении:**

После настройки coturn сервера, добавьте credentials в `.env` файл:

```env
VITE_TURN_SERVER_URL="turn:YOUR_SERVER_IP:3478"
VITE_TURN_SERVER_USERNAME="username"
VITE_TURN_SERVER_CREDENTIAL="password"

# Опционально: добавьте TCP транспорт для надежности
VITE_TURN_SERVER_URL_2="turn:YOUR_SERVER_IP:3478?transport=tcp"
VITE_TURN_SERVER_USERNAME_2="username"
VITE_TURN_SERVER_CREDENTIAL_2="password"
```

**Для TLS (порт 5349):**

```env
VITE_TURN_SERVER_URL="turns:YOUR_SERVER_IP:5349"
VITE_TURN_SERVER_USERNAME="username"
VITE_TURN_SERVER_CREDENTIAL="password"
```

---

## 🧪 Тестирование соединения

### Сценарии для тестирования

1. **Оба в одной сети**
   - Ожидаемый результат: прямое P2P соединение (host candidates)
   - Тест: откройте приложение на двух устройствах в одной Wi-Fi сети

2. **Один пользователь за VPN**
   - Ожидаемый результат: использование TURN relay
   - Тест: включите VPN на одном устройстве
   - Проверьте в консоли: должны появиться "relay" candidates

3. **Оба за VPN или мобильным интернетом**
   - Ожидаемый результат: использование TURN relay
   - Тест: оба пользователя с VPN или один на мобильном 4G/5G
   - Критически важен работающий TURN сервер

4. **Разные страны/континенты**
   - Ожидаемый результат: может использоваться как P2P, так и relay
   - Тест: попросите коллегу/друга из другой страны протестировать

### Диагностика проблем

**Откройте консоль браузера (F12) и проверьте логи:**

```
✅ Хорошие признаки:
- "TURN relay candidate gathered"
- "relay" в candidate breakdown
- "Connection established successfully"

🔴 Проблемные признаки:
- "NO RELAY CANDIDATES GATHERED"
- "ICE connection failed"
- "All connectivity checks failed"
- Только "host" и "srflx" кандидаты без "relay"
```

**Типичные ошибки и решения:**

| Ошибка | Причина | Решение |
|--------|---------|---------|
| NO RELAY CANDIDATES | TURN серверы недоступны | Проверьте credentials, смените на платный сервис |
| ICE connection failed | Все кандидаты отклонены | Включите TURN relay, проверьте firewall |
| Connection timeout | Signaling проблема | Проверьте Supabase подключение |
| Works without VPN, fails with VPN | TURN не настроен правильно | Используйте надежный TURN сервис |

---

## 📊 Мониторинг и отладка

### Логирование в приложении

Приложение автоматически логирует:
- Типы собранных ICE кандидатов
- Активные пары кандидатов
- Тип соединения (P2P/relay)
- Статистику connection/ICE состояний

### Полезные команды для диагностики

```javascript
// В консоли браузера:

// 1. Посмотреть все candidates
peerConnection.getStats(null).then(stats => {
  stats.forEach(report => {
    if (report.type === 'local-candidate') {
      console.log(report);
    }
  });
});

// 2. Посмотреть активное соединение
peerConnection.getStats(null).then(stats => {
  stats.forEach(report => {
    if (report.type === 'candidate-pair' && report.state === 'succeeded') {
      console.log('Active pair:', report);
    }
  });
});

// 3. Проверить используется ли TURN
peerConnection.getStats(null).then(stats => {
  stats.forEach(report => {
    if (report.type === 'candidate-pair' && report.state === 'succeeded') {
      console.log('Using relay?', 
        report.localCandidate?.candidateType === 'relay' || 
        report.remoteCandidateType === 'relay'
      );
    }
  });
});
```

---

## 🔐 Безопасность

### Рекомендации:

1. **✅ Используйте .env для credentials** - приложение уже настроено для использования переменных окружения:
   ```env
   VITE_TURN_SERVER_URL="turn:your-server.com:443"
   VITE_TURN_SERVER_USERNAME="your-username"
   VITE_TURN_SERVER_CREDENTIAL="your-password"
   ```
   
   Credentials автоматически читаются из `.env` файла в `VideoCall.tsx`:
   ```javascript
   const turnServerUrl = import.meta.env.VITE_TURN_SERVER_URL;
   const turnUsername = import.meta.env.VITE_TURN_SERVER_USERNAME;
   const turnCredential = import.meta.env.VITE_TURN_SERVER_CREDENTIAL;
   ```

2. **🔒 Защита .env файла:**
   - `.env` уже добавлен в `.gitignore` - никогда не коммитьте его в git
   - Используйте `.env.example` как шаблон для команды
   - Для production настройте переменные окружения в панели хостинга (Vercel, Netlify, etc.)

3. **⏱️ Используйте временные credentials** (advanced):
   - Для максимальной безопасности генерируйте короткоживущие токены на сервере
   - Twilio и некоторые другие провайдеры поддерживают эфемерные токены
   - Регулярно ротируйте credentials

4. **🛡️ Ограничьте доступ к TURN** (для собственного coturn):
   - Настройте IP whitelist
   - Включите rate limiting
   - Используйте strong passwords или генерируйте их автоматически
   - Ограничьте max-bps и total-quota в конфигурации coturn

5. **📊 Мониторинг использования:**
   - Следите за трафиком и стоимостью
   - Настройте алерты при превышении лимитов
   - Регулярно проверяйте логи на подозрительную активность

6. **🔄 Для production деплоя:**
   - Vercel: Settings → Environment Variables → добавьте `VITE_TURN_SERVER_*`
   - Netlify: Site settings → Build & deploy → Environment → добавьте переменные
   - Custom VPS: Настройте `.env` файл на сервере с правильными permissions (`chmod 600 .env`)

---

## 🌍 Выбор TURN провайдера по регионам

| Регион | Рекомендация | Причина |
|--------|--------------|---------|
| 🇷🇺 Россия | Собственный coturn на VPS | Многие зарубежные сервисы заблокированы |
| 🇪🇺 Европа | Metered.ca, Xirsys | Хорошая скорость и надежность |
| 🇺🇸 США | Twilio, Metered.ca | Enterprise поддержка |
| 🇨🇳 Китай | Собственный coturn | Great Firewall блокирует многие сервисы |
| 🌏 Азия | VideoSDK, Metered.ca | Серверы в регионе |

---

## 🚀 Деплой

После настройки TURN серверов приложение готово к деплою. Убедитесь что:

✅ Используете надежный TURN сервис (не бесплатные публичные)  
✅ Credentials храните в переменных окружения  
✅ Протестировали все сценарии (VPN, NAT, мобильный)  
✅ Настроили мониторинг использования TURN  
✅ Подготовили план масштабирования для высокой нагрузки

---

## 📚 Полезные ресурсы

- [WebRTC документация](https://webrtc.org/getting-started/overview)
- [Понимание ICE, STUN и TURN](https://webrtc.org/getting-started/peer-connections)
- [coturn проект на GitHub](https://github.com/coturn/coturn)
- [Metered.ca документация](https://www.metered.ca/docs)
- [Xirsys документация](https://docs.xirsys.com/)
- [Twilio Network Traversal](https://www.twilio.com/docs/stun-turn)

---

## ❓ FAQ

**Q: Почему мое соединение работает иногда, а иногда нет?**  
A: Скорее всего бесплатные TURN серверы перегружены или недоступны. Переходите на платный сервис.

**Q: Нужен ли TURN если пользователи в одной сети?**  
A: Нет, для одной сети достаточно STUN. Но для надежности лучше иметь TURN как fallback.

**Q: Сколько стоит TURN для 1000 пользователей?**  
A: Зависит от длительности звонков. ~100-200$/месяц для средней нагрузки (Metered.ca). Или ~50$/месяц за VPS для своего coturn.

**Q: Можно ли использовать несколько TURN серверов?**  
A: Да! Добавьте несколько серверов в массив `iceServers` для резервирования.

---

## 📝 Лицензия

MIT

---

Сделано с ❤️ для надежной WebRTC коммуникации
